...
function fetchTopStocks() {
    // Fetch data from api
    fetch(`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=IBM&apikey=YIANOWEWQ4G6PPSM`).then(response => response.json()).then(data => {
        const stocks = data['Rank A: Real-Time Performance']; // Verify the correct key
        let html = '';
...
// Function to fetch and display stock data for the search symbol
function fetchStockData(symbol) {
    ...
    fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=YIANOWEWQ4G6PPSM`).then(response => response.json()).then(data => {
